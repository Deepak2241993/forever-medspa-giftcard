...
class StripeController extends Controller
{
    ...
    public function CheckoutProcess(Request $request)
    {
        ...
        try {
            ...
            // Handle gift card applied scenario
            if (session()->has('total_gift_applyed')) {
                ...
                // Calculate total amount for non-gift-card scenario
                $cards = session('cart', []);
                foreach ($cards as $item) {
                    $cart_data = ($item['type'] === 'product') 
                        ? Product::find($item['id']) 
                        : ServiceUnit::find($item['id']);
    
                    $totalAmount += $item['quantity'] * ($cart_data->discounted_amount ?? $cart_data->amount);
                }
            ...
            // Prepare data for `transaction_histories`
            $data = [
                'fname' => $request->fname,
                'lname' => $request->lname,
                'city' => $request->city,
            ...
            // Insert and update transaction history
            $latestId = DB::table('transaction_histories')->insertGetId($data);
            $orderId = 'FMSWCSU' . str_pad($latestId, 8, '0', STR_PAD_LEFT);
            DB::table('transaction_histories')->where('id', $latestId)->update(['order_id' => $orderId]);
                ...
                'line_items' => [
                    [
                        'price_data' => [
                            ...
                            'unit_amount' => session('totalValue', $final_amount) * 100,
                            'currency' => 'USD',
                        ],
                        'quantity' => 1,
                    ],
    ...
    public function stripcheckoutSuccess(Request $request)
{
    try {
        ...
        $transaction_data = \App\Models\TransactionHistory::where('payment_session_id', $response->id)->first(); 
         \App\Models\ServiceOrder::where('order_id', $transaction_data->order_id)
        ->update(['status' => 1]);
        if ($transaction_data) {
